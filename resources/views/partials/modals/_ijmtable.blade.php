    ...
    <div class="-m-1.5 overflow-x-auto">
        <div class="p-1.5 min-w-full inline-block align-middle">
            <div class="overflow-hidden">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="px-6 py-1 whitespace-nowrap text-sm text-center  font-medium text-gray-900 border border-gray-400"><span class="font-bold text-red-500" id="no_remaining_accomodation">
                                {{ $missionOrder->no_accomodation - $missionOrder->no_ded_accomodation }}</span></td>
                            ...
                            <td class="px-6 py-1 whitespace-nowrap text-sm font-medium text-gray-900 border border-gray-400">
                                {{ $missionOrder->no_accomodation * $missionOrder->bareme->accomodation_cost }}
                            </td>
                            <td class="px-6 py-1 whitespace-nowrap text-sm font-medium text-gray-900 border border-gray-400">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-1 whitespace-nowrap text-sm text-center  font-medium text-gray-900 border border-gray-400"><span class="font-bold text-red-500" id="no_remaining_meals">
                                {{ $missionOrder->no_meals - $missionOrder->no_ded_meals}}</span></td>
                            <td class="px-6 py-1 whitespace-nowrap text-sm font-medium text-gray-900 border border-gray-400">
                                {{ $missionOrder->no_meals * $missionOrder->bareme->meal_cost }}
                            </td>
                            <td class="px-6 py-1 whitespace-nowrap text-sm font-medium text-gray-900 border border-gray-400">
                                {{ $missionOrder->bareme->currency }}</td>
                        </tr>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th scope="col"
                                ...
                                {{ $missionOrder->no_accomodation * $missionOrder->bareme->accomodation_cost +  $missionOrder->no_meals * $missionOrder->bareme->meal_cost}}
                                <input type="hidden" name="total_amount" value="{{ $missionOrder->no_accomodation * $missionOrder->bareme->accomodation_cost +  $missionOrder->no_meals * $missionOrder->bareme->meal_cost}}">
                            </th>
                            <th scope="col"
                                class="px-6 py-1 text-start text-xs font-bold text-blue-600 uppercase border border-gray-500">
                ...
                <script>
                    document.getElementById('no_remaining_accomodation').textContent = {{$missionOrder->no_accomodation}}-document.getElementById('no_ded_accomodation').value;
                    document.getElementById('no_remaining_meals').textContent = {{$missionOrder->no_meals}}-document.getElementById('no_ded_meals').value;
                    document.getElementById('no_ded_accomodation').addEventListener('input', function(){
                        document.getElementById('no_remaining_accomodation').textContent  = {{$missionOrder->no_accomodation}}-this.value;
                    });
