<div id="approveModal-{{ $missionOrder->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="flex justify-between items-center p-2 rounded-t border-b">
                <div class="text-base font-bold mt-3 sm:mt-0 sm:ml-4 sm:text-left blue-color">
                    {{ __('Approve MÉMOIRE DE FRAIS	') }}:{{ $missionOrder->order_number }}:{{ $missionOrder->purpose }}
                </div>
                <div>
                    <button type="button"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                        data-modal-toggle="approveModal-{{ $missionOrder->id }}">
            ...
            <div class="p-4 overflow-y-auto" style="max-height: 700px">
                <form method="POST"
                    action="{{ route('mission_approves.m_approve', $missionOrder->id) }}">
                    @csrf
                    ...
                    <div class="flex justify-end items-center space-x-2 rounded-b ">
                        <div>
                            <button data-modal-toggle="approveModal-{{ $missionOrder->id }}"
                        ...
                        <div>
                            <button name="action" value="approve"
                                ...
                                data-modal-toggle="approveModal-{{ $missionOrder->id }}">{{ __('Approve (GO Next!)') }}
                            </button>
                        </div>
                        <div>
                            <button name="action" value="reject"
                                ...
                                data-modal-toggle="approveModal-{{ $missionOrder->id }}">{{ __('Reject (End process)') }}
                            </button>
                        </div>
                    </div>
                </form>
